    ...
    <div>
                ...
                <tr v-for="(rowHeadValue, rowHeadIndex) in rowHead" :key="rowHeadValue">
                  ...
                  localColumns.length + (localValues.length ? 1 : 0) + rowHeadIndex
                    ...
                    <template v-for="(rowValue, rowValueIndex) in tableData[rowHeadIndex]">
                        <td v-for="(value, index) in emptyLocalValues.length
                  ? emptyLocalValues
                  : localValues" :key="'' + rowHeadIndex + rowValueIndex + index">
                            <div>
                                {{ rowValue[value.key] }} [{{
                    localColumns.length +
        ...
        <table>
            <tr v-for="(tr, trIndex) in result" :key="trIndex">
                <td :rowspan="td.rowspan" :colspan="td.colspan" v-for="(td, tdIndex) in tr" :key="tdIndex">
    ...
    export default {
        ...
        methods: {
            ...
            setValuesToColAndRow() {
                const columnKeys = this.localColumns.map(({
                    key
                }) => key);
                const rowKeys = this.localRows.map(({
                    key
                }) => key);
            ...
            handleFormatData2() {
                ...
                const mergeInfo = (info = {}) => Object.assign({}, baseCellInfo, info);
                ...
                return rowHeadConditions.map((row, rowIndex) =>
                    colHeadConditions.map((col, colIndex) => {
                        ...
                        if (
                            ...
                            // cellData.__empty = "";
                            cellData.push(mergeInfo({
                                x: this.localColumns.length +
                                    (this.localValues.length ? 1 : 0) +
                                    rowIndex,
                                y: this.localRows.length + colIndex,
                                value: "",
                                isSummary: conditionKeys.length !== Object.keys(conditions).length,
                            }));
                        } else {
                            cellData = this.values.map((item, index) => {
                                return mergeInfo({
                                    x: this.localColumns.length +
                                        (this.localValues.length ? 1 : 0) +
                                        rowIndex,
                                    y: this.localRows.length + colIndex * 2 + index,
                                    value: item.handle ? item.handle(filterData) : "",
                                    isSummary: conditionKeys.length !== Object.keys(conditions)
                                        .length,
                                });
                                // cellData[item.key] = item.handle(filterData);
                            });
            ...
            handleDataViews() {
                ...
                const rowsHead = this.localRows.map((row, index) => {
                    return mergeInfo({
                        value: row.label,
                        y: index,
                        rowspan: (this.localColumns.length + (Number(Boolean(this.localValues
                            .length)))) || 1
                    });
                });
                    ...
                    columnsHead.forEach((row, rowIndex) => {
                        row.push(
                            mergeInfo({
                                ...
                                x: rowIndex,
                                y: rowIndex === 0 && this.localRows.length ?
                                    this.localRows.length + colIndex : colIndex,
                                colspan: this.localValues.length || 1,
                                isSummary: colArr.length !== this.localColumns.length,
                            })
                        );
                    });
                });
                    ...
                    this.localValues.forEach((cell, cellIndex) => {
                        ...
                        valuesHead.push(
                            mergeInfo({
                                value: cell.label,
                                x: this.localColumns.length,
                                y: (this.localColumns.length ? 0 : this.localRows.length) +
                                    i * 2 +
                                    cellIndex,
                                isSummary: col ?
                                    col.split(this.SEPARATOR).length !== this.localColumns.length :
                                    false,
                            })
                        );
                    });
                ...
                const finalData = rowsValue.length ? rowsValue.map((row, index) => [].concat(row).concat(data[index])) :
                    data;
            ...
            _compileHead(...args) {
                ...
                if (_isHasSummary) {
                    ...
                    for (let i = args.length - 1; i > 0; i--) {
                        totalKeys = totalKeys.concat(this._combine(...args.slice(0, i)));
                    }
